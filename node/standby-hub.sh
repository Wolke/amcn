#!/bin/bash
# 待命排序器（**舊路徑，#95 之後只在一種情況下才用它**）。
#
# 現在的做法是 `node standby.js <rendezvous 位置> --pin did:demo:<現任>`：
# 它自己拉帳、自己驗、現任連續連不上就自己升格，而且**不需要現任的私鑰**
# （靠現任事先簽好的接手憑證，client 釘的仍然是前任）。閘門 demo-succession.js 9/9。
#
# 這支留下來的唯一理由是「我就是要用同一個 DID 接手」——例如演練時要讓
# client 完全感覺不到換人。代價寫在下面那一段，而那個代價是真的。
#
