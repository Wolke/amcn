...
- **額度是賺來的，不是給的。** 新身分第一天約 **4.6 CC**（starter 10 × 新戶品質係數
  0.925 × 年齡斜坡 0.5，30 天後約 9.3），而不足的那部分由**入門採購**補上：營運方的
  Treasury 會向還沒賺過 CC 的身分買一份**答案已知**的工作（連續 3 次通過才付一次，
  每身分上限 20 CC）。也就是說**你的第一筆 CC 是賺來的，不是領來的**——這是刻意的：
  送無擔保額度的話，攻擊者鑄造 N 個身分就是 N 份禮物（#90／#50 量過）。
  舊的說法（starter 50、第一天 23.1 CC）在 2026-09-23 之後不再是預設，
  理由與數字見 `docs/evaluation/credit-regime-ab.md`。

  <details><summary>舊版說明（starter 50 的體制）</summary>

  新身分第一天約 **23.1 CC**（starter 50 × 新戶品質係數 × 年齡斜坡的前半），
  30 天後才走到約 46.25 CC；之後隨成交紀錄與**對手多樣性**成長。

  </details>

- **額度隨成交紀錄與對手多樣性成長**，這一條沒變。
...
**什麼都不用做**，前提是你用的是位址記錄（`rendezvous`／`rv:`）而不是寫死的位址。

排序器可以被授權接手：現任趁自己還活著時簽一份「誰可以接手我」，憑證隨位址記錄
發布。現任消失後，待命機自己拉帳、自己驗、自己升格，並用**它自己的** DID 服務；
你這一側每次重連都會重新解析記錄，看到的是

```
rendezvous → 10.0.0.9:47180 (did:demo:6126098b8f9e…, 3s old) — 接手：did:demo:01c5f6ee897… 事先授權了它（1 段委派）
```

你釘的仍然是**前任**，而你跟過去的理由是前任事先簽過的授權——**沒有任何私鑰被
複製**，所以「能接手」不等於「拿到了排序器的鑰匙」。沒有那條授權鏈的人，你的節點
不會跟（實測：它會說 `not the pinned hub，而附帶的接手憑證裡沒有一條從它出發的鏈`）。

接手之後那本帳裡會有**前任簽的** checkpoint，而 `verify-ledger.js` 仍然驗得過：
委派鏈跟著匯出走，所以合法的簽署者是「現任＋能走到現任的每一個前任」。

閘門 `node demo-succession.js` 9/9（含三條負對照）。**沒有宣稱的**：這是有紀錄的
failover，不是共識——兩個後繼者同時升格就是分叉，偵測有、自動合併沒有。
