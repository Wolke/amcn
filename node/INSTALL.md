...
### 新人的第一筆額度：送一點點，其餘去買（#90 的 D 組，2026-09-23）

`./install.sh` 現在裝**五個**服務，第五個是入門採購的發樁者（`com.amcn.onboard`）：
Treasury 向**還沒賺過任何 CC 的身分**購買一份**答案已知**的工作。預設值是量出來的：

| | 白拿／身分 | 壞帳 vs 風險費收入（受攻擊時）| 結算量 |
|---|---:|---:|---:|
| starter 50（舊預設）| **44.17** | 889 vs 450 ← 保險池被打穿 | 100% |
| starter 0、什麼都不做 | 0 | 0 vs 38 | **7.7%**（市場塌了）|
| **starter 10 ＋入門採購（現在的預設）**| **8.56** | 171 vs 201 ← 蓋住了 | 45% |

白拿 ≈ starter × 0.87，所以**這個值就是你對 Sybil 的曝險上界**——一旦
`network.json` 公開，任何人都可以鑄造身分。全表與四種體制的對照在
`docs/evaluation/credit-regime-ab.md`。

```bash
./install.sh status           # 會列出五個服務與發樁者的 DID
./service/run-onboard.sh --did   # 發樁者的身分（Hub 授權的就是它）
DEMO_STARTER_CC=50 ./install.sh  # 改回舊值（等於接受每個新身分 ~44 CC 的禮物）
```

兩個治理上限預設 **每身分 20 CC／全網 2,000 CC**（`HUB_ONBOARD_CAP_CC`、
`HUB_ONBOARD_TOTAL_CC`），而付款要**連續 3 次通過驗收**才發一次
（`HUB_ONBOARD_STREAK`）——實測攻擊者每身分從 5.52 CC 降到 0.00，而誠實新人只從
196 人掉到 187 人。**順序很重要**：`install.sh` 會在起 Hub 之前先產生發樁者身分，
因為 Hub 是讀 `configs/.onboard-seed` 算出 DID 才能授權它。反了的話發樁者的每一筆
都被拒絕，而那看起來只像「新人沒有工作可做」。閘門 `node demo-service.js` 6/6。

**信任邊界要講清楚**：發樁者是營運方授權的身分，所以它**能夠多報失敗來卡住某個
新人**。那個權力本來就存在（它也可以乾脆不發任務給你），制衡是它的支出全部在帳
上、可稽核、有治理上限。
