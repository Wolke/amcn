...
| **Credit Line** | C 案（修數字） | INV-C1：`credit_limit ≤ k_earn × E_eff + collateral + L_boot`，**L_boot 明文列為不變式的有界例外**（**上限 50 CC**、Treasury 補貼科目、tx_class=subsidy）；E_eff＝多樣性折減（單一對手 ≤20%）× T_flow 信任流加權；A 案的 WilsonLower95＋1−HHI 併入 E_eff 實作 | GATE-0 已定案 50 CC（見 #1 裁決）。**上限先前寫 25 CC 是本表的錯誤**：它來自 proposal-C 的草稿值，而 GATE-0 掃描 starter ∈ {10,20,50} 的結論相反 |
...
| # | 來源 | 缺陷 | 整合修法 |
|---|---|---|---|
| 1 | B①/C③ | 新戶信用額度數字三處打架；INV-C1 被 L_boot 形式違反 | **已裁決：L_boot = 50 CC**（2026-09-16）。依據本欄原本的修法——「單一數值表由 GATE-0 模擬定案」——而 GATE-0 的證據與 §2.2 先前寫的 25 CC 相反，且方向反直覺：`sim/out/sweep.csv` 掃描 starter ∈ {10,20,50} × 費率 × deadbeat ∈ {5,15,30}%，**提高 starter 反而降低壞帳率**（30% deadbeat 下 starter 10→50 使壞帳 5.27%→2.23%），因為流動性成長（結算量 5.4k→32.9k CC，6 倍）快過壞帳絕對值，違約損失被市場規模稀釋；starter 10 同時餓死市場又放大壞帳占比（`phase0-results.md` §結論 2/5）。三處現已一致：§2.2 改為 50、`node/lib/eeff.js` 的 `STARTER_CC` 本來就是 50、模擬器同值。**t=0 的實際額度是 46.25 CC**（50 × quality prior 0.925），這個差異是刻意的，不是誤差。**仍未關閉的兩件事另行登記**：(a) L_boot 的 Sybil 維度沒有證據，見 #50；(b) `tx_class=subsidy` 科目在 node 端完全未實作（原型沒有 tx_class），屬 FR-083／§20-9 範圍 |
