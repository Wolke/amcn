...
| 18 | **P1→撤銷** | 原登記：跨機 UDP 發現無法使用。**已證否**——`discovery-probe.js` 在原本失敗的那台實測：真實 hub_beacon 跨機抵達、簽章 OK、時差 1020ms（容許 60s），且 `resolve` 模式（呼叫 agent 使用的同一個 `resolveHubTarget`）成功發現 Hub。**登記本身有瑕疵**：當初是從「機器 2 未出現在註冊清單」**推論**發現失敗，從未取得該機的 `no hub beacon heard` log，commit message 還寫入未經驗證的成因（AP isolation 等）。仍然成立的部分：`demo.js` 的發現檢查確實只驗同機，`af5836a` 宣稱「Verified on the real LAN」為錯誤陳述，該檢查已更名。診斷工具保留，若再發生可用 `listen`／`resolve` 定案 |
...
| 33 | **P0** | 協議訊息完全沒有版本欄位，混版節點以**崩潰**而非拒絕收場。實測：試點中機器 1 的 pre-W9 provider 收到機器 2 的 W9 合約（帶 `verifier_pool`、無 `verifiers`）後，在 `c.verifiers.length` 拋例外並死在合約中途，requester 同時死亡。proposal-B §9 要求所有 schema 帶 `"v": 1`（NFR-004 亦然），原型一個都沒有 | 未修。需在 register/contract/receipt/attestation 加 `v` 欄位，並在 Hub 註冊時協商；在那之前，**升級必須所有機器同時進行**（INSTALL 已加註）|
...
| 28 | **P2** | 模擬器的押注並未真正託管：slashing 取 `min(stake × 10%, 餘額)`，所以 fee 0% 時完全無法沒收（掃描首四列的沒收額皆為 0）。真實設計應要求 Verifier 先把押金從自身餘額轉入 `protocol:stake` | 未修。影響：目前的威懾強度與費率耦合，費率越低威懾越弱，這個耦合是實作產物而非設計意圖 |
| 29 | **P2** | 金絲雀比例的單位與提案不符：proposal-C §7 指定金絲雀占**全網量** 2–5%，模擬器的 `canary_rate` 是每 tick 注入機率，實測只產生 0.5–1.3% 的量。要達到提案的 2–5% 需大幅提高比例，而那會把 #27 的誠實者沒收成本按比例放大 | 未修。需先定義單位，再重跑掃描 |
| 30 | **P2** | 偷懶者被抓率（50–87.5%）與 pool 大小強耦合：模擬只有 9 個 Verifier，每個都看到大量金絲雀。大型網路中個體暴露被稀釋，此數字不可外推——#8 對「抽查率 0.13% 撐不起威懾」的擔憂在規模化後可能依然成立 | 未修。需以 pool 大小為掃描維度重測 |
