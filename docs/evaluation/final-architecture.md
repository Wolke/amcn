...
| 40 | **P0** | **沒有任何重連機制**：`resolveHubTarget` 只在啟動時解析一次，`dialLazy` 取得 channel 後不再管它。Hub 一消失，`chan.send()` 只回傳 `false`，而所有呼叫端都忽略回傳值——agent／verifier／canary 從此**靜默失聯**：不重連、不重新發現、不出聲，Console 仍顯示最後已知餘額。發現時機：評估 W10 拔線演練可行性時追「Hub 被拔線後 agent 會怎樣」。連帶後果：#14 交付的「發現與輪替」在運行期不可用——輪替只有在**所有進程同時重啟**時才生效，而這正是輪替該避免的事；#19 的「WARNING: not connected」只覆蓋啟動期，斷線後不會再說話 | 未修。修法：`dialLazy` 加重連退避＋重連後重發 `register`（Hub 端 #35 已支援同 DID 重註冊沿用 stats），斷線／重連皆須 log，並讓 `send()` 在無連線時排隊而非丟棄（排隊上限須有界，否則離線期間的送出會無限堆積）。**這是 W10 拔線演練的前置**：沒有它，演練量到的是「人手動重啟 N 個進程要多久」 |
| 41 | **P1** | 自動匯出的成本隨歷史無上限成長：`rawLog` 保留 Hub 收到的**每一行**（NFR-005 明文掃描需要），而它進 `buildExport()`，且 `HUB_DUMP_PATH` 每 10 秒把整份匯出重寫一次。`demo.js` 跑 9 秒就累積 38.5 KB，所以長時間執行時匯出本身會變成主要成本（每次都 `JSON.stringify` 整段歷史，並在事件迴圈上阻塞）。所有 demo 都只跑 15–35 秒，這個量級從未被觸及 | 未修。選項：(a) `rawLog` 設環狀上限（明文掃描只需要近期窗口）；(b) 把 `raw_log` 移出 dump，只留在 `export` 回應中；(c) 改增量匯出。需先量測真實試點速率下的成長曲線，再決定——#39 的教訓：沒量過的數字不要當結論 |
