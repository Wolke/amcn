...
| 61 | **P1** | **手續費單向匯出 ＋ 反洗量折減，會讓小型封閉經濟必然鎖死**。40 分鐘無人 soak 實測：292 筆結算後，三個交易者餘額合計 **−122.53 CC**、protocol 帳戶合計 **+122.53**（保險 75.90、Treasury 31.63、押注 15.00），三人全部貼著各自的信用上限（−52.2/54.8、−52.7/54.1、−53.2/55.2），**最後 12 分鐘零成交**。兩個機制疊加：每筆交易 8.5% 永久離開流通（Treasury 2.5%＋保險 6%，另加驗證費託管），而 F-1 的反洗量折減（單一對手 ≤ 其他對手加權量的 20%）在只有三個交易者時幾乎把 E_eff 壓成零——額度只從 50 爬到 54–55，抽乾速度快過額度成長。**模擬器有 `write_off`（保險吸收違約），原型一條回流路徑都沒有**；而 §2.2「Treasury 啟動」設計的回流（金絲雀、逆週期收購、L_boot 補貼）在 soak 中一個都沒啟用 | 未修，且**不該直接在原型裡改**——本專案的規則是經濟參數以模擬為回歸閘門。這條的真正問題是「最小可行網路規模」：GATE-0 跑 400 個 agent 時多樣性足夠，這個效應應該弱得多，但**從來沒有人量過它與 N 的關係**。下一步是模擬掃描 N × 費率 → 觸底時間，而不是在 node 裡調參。短期可操作的是 (a) 讓金絲雀在長跑情境中啟用（它已實作且會花用 Treasury），(b) 指標把 protocol 帳戶與交易者分開計算 |
