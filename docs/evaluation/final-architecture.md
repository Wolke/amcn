...
| 27 | **P1** | 金絲雀懲罰誠實者的一般能力誤差：在**零偷懶者**的情境下，fee 4%＋canary 3% 仍沒收 20.21 CC （canary 5% 時 41.19 CC），約佔 verifier 總收入 4.6%。competence 0.94–0.995 意味著在已知錯誤的任務上誤判 PASS 是必然會發生的，而 proposal-C 的設計「canary_failed 即沒收押注 10%」沒有區分誤差與作弊 | 未修。單次失敗即沒收過於敏感；應改為門檻式（例如觀察窗內 k 次失敗、或失敗率顯著高於族群基準才沒收）|
