...
| 20 | **P1** | W7 的還債策略參數三處不一致：還債折價 proposal-B §8.4 寫 10%、`sim/amcn_sim/market.py` 一直用 35%、node 原型照提案實作 10%；band 下界 §8.4 寫 −0.3×CL、模擬器用固定 −5.0 CC；band 上界 §8.4 寫 +100、模擬器用每 agent `uniform(30,120)`。同 #1「新戶信用額度數字三處打架」的形態 | 處理中：`market.py` 已參數化（`repay_discount`、`band_low_cl_frac`），新增 `amcn_sim.sweep_repay` 掃描 折價 × band 定義 × 情境，以 GATE-0 判準＋Credit Velocity 選值。CLAUDE.md 定模擬器為經濟回歸閘門，定案後 node 端 `lib/strategy.js` 必須對齊 |
