...
**三台機器全部必須跑同一個 commit。** 協定現在是 **v7**（2026-09-22 的 `stake_release`，#38；v6 是 #41 把 `cp.heads` 移出簽署內容），而版本不符會被明確拒絕（`#33` 的版本閘門），不會靜默半通。先在三台上 `git log --oneline -1` 對過再開始。
...
2026-09-21 實測：套件跑到第 12 項 `tail-recover` 時，正好落在拔線的那幾分鐘，結果 **1/4**——而同一支在安靜的機器上連跑兩次都是 **4/4**，負向對照 2/4 不變。chaos 的 hub 啟動時會列舉網路介面來算廣播位址（`beaconTargets`），介面消失就會擾動它。

**那是假紅，但當下分不出來**，而分不出來的代價是：你會以為剛改的東西壞了，然後往錯的方向查。演練期間那台機器只做演練。

（同一天還踩過另一個：忘了 `DEMO_PORT_OFFSET`，讓 `demo.js` 綁上 `127.0.0.1:47180` 與跑中的試點撞埠，於是本機查詢全部被 demo 的 hub 接走——**在 macOS 上綁 `127.0.0.1` 的 socket 會優先於綁 `*` 的**。在跑著試點的機器上，本機查詢一律走區網位址。）
...
| 演練後 | **接回網路之前，先停掉 M1 的排序器進程** | 2026-09-21 實測踩到：演練結束後把 M1 插回來，而它的 Hub 仍在執行，於是同網段短暫存在兩個同 DID 的排序器。那次沒有分岔純屬運氣——M1 的 verifier 全部移到 M2，沒有 panel 就無法驗收，所以它產不出任何新結算。只要它保住一個 verifier，兩邊都能結算，那就是真的分叉。原本只寫「不得再接回」，但演練總是要結束的，沒寫「接回前先停什麼」等於沒寫 |
