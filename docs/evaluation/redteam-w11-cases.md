...
| E3 | 威脅 7 | 三身分環狀洗量（A→B→C→A），規避成對偵測 | **block（環狀）／開（團狀）** | 已量測（`node/wash-sweep.js`，#70）：環狀與星狀有效率 **0.0%**，完全被壓平；但**團狀**（每人付每人）從 10% 升到 N=24 的 **95.7%**，成對折減漸近失效。擋住它的是手續費而不是 F-1，餘裕僅 1.20× | 自動 |
...
| F8 | #10 | 偽造 checkpoint 簽章（**離線**重建路徑）| block | `rebuild()` 以 `hub_pub` 驗章；竄改即拒絕啟動。**注意這一案只覆蓋離線那條路**——活著的節點原本不驗，見 F9／#75 | 自動 |
| F9 | #75 | 偽造 checkpoint 簽章**餵給活著的節點**（cp 內容全真、只換簽章）| ~~known-open~~ **block** | 已修：`registered` 交出 `hub_pub`，`cpwatch.observeEntry()` 驗不過就丟且不進比對基準（`HUB_CP_FORGE=all`；實測拒絕 10 份、採信 0 份、誤報 0 次）| 自動 |
| F9b | #75 | 同上，攻擊期間的帳本不變式 | block | 七項不變式全程成立——偽章擋住的是 panel 種子污染，代價是種子等不到（同 #72，非緩解）| 自動 |
...
| G14 | #16 | 對缺必要欄位的 frame 逐欄位驗證 | ~~known-open~~ **block（2026-09-18 交付）**| 派發前檢查每個型別「handler 會直接解參照」的欄位，指名缺什麼。紅隊現為 **block 41、known-open 0** | 自動 |
...
| **E3** | Sybil | **環狀 0.0%、團狀 95.7%** → #70（P1）。盤點表寫「block？…未驗證」，量完發現直覺的規避法（環狀）完全無效，而沒人想過的團狀幾乎全漏；但槓桿處處 < 1，**擋住它的是手續費不是 F-1**，餘裕只有 1.20× |
