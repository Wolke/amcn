...
> （quickstart 起的是你自己的島，所以它用的是自己的參數；上面那個 25.0／23.1 的
> 額度是 starter 50 的舊輸出——常駐服務的預設已經是 starter 10 ＋入門採購。）
...
- **額度是賺來的**。新身分第一天只有約 4.6 CC（starter 10），不足的部分由營運方的
  Treasury **購買**你的第一份工作來補（答案已知、連續 3 次通過才付，#90）——送無擔保
  額度的話，攻擊者鑄造 N 個身分就是 N 份禮物。之後隨成交紀錄與**對手多樣性**成長；
  跟自己刷量沒用：實測從同一個對手賺 36.6 CC，額度零成長。
